...
'cms':{//系统
	...
	'pre':function(){//代码高亮
		...
		if($("pre code").length){
			...
			$("<link>").attr({ rel: "stylesheet",type: "text/css",href: "//cdn.jsdelivr.net/gh/duwa12/js@main/googlecode.min.css"}).appendTo("head");
			$.getScript("//cdn.jsdelivr.net/gh/duwa12/system-public@main/highlight.min.js", function(){
...
'language':{//简繁转换
	's2t':function(){
		if(feifei.browser.language=='zh-hk' || feifei.browser.language=='zh-tw'){
			$.getScript("//cdn.jsdelivr.net/gh/duwa12/system-public@main/s2t.min.js", function(data, status, jqxhr) {
...
'image': {//图片
	...
	'qrcode': function(){//生成二维码
		...
		$(".glyphicon-phone").on('show.bs.popover', function () {
			$(".glyphicon-phone").attr('data-content','<img class="ff-qrcode" src=""/>');
		})
	},
	'vcode':function(){//安全码
...
'score': {//评分
	'raty': function(){
		if( $('.ff-score').length ){
			...
			$("<link>").attr({ rel: "stylesheet",type: "text/css",href: "https://cdn.jsdelivr.net/gh/duwa12/system-public@main/jquery.raty.min.css"}).appendTo("head");
			//
			$.getScript("https://cdn.jsdelivr.net/gh/duwa12/system-public@main/jquery.raty.min.css", function(response, status) {
